(function(){
  ...
  async function supervisor(){
    if(supervisorStarted||!('serviceWorker' in navigator))return;const c=window.supabase.createClient(cfg.url,cfg.publishableKey),{data}=await c.auth.getSession(),session=data&&data.session;if(!session)return;
